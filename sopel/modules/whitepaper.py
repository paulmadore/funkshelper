...
@commands('halvening', 'holiday')
def halvening(bot, trigger):
    bot.reply('The following are the Official Woodcoin Holy Days, as decreed by funkenstein_ on 28 March 2016:')
    bot.say('Pi day || Reward drops below pi LOG || 31 March 2016')
    bot.say('3 day || Reward drops below 3 LOG || 28 April 2016')
    bot.say('e day || Reward drops below e LOG || 8 June 2016')
    bot.say('2 day || Reward drops below 2 LOG || 8 December 2016')
    bot.say('gold day || Reward dropbs belw phi (~1.618) LOG || 22 May 2017')
    bot.say('sqrt(2) day || Reward drops below sqrt(2) LOG || 22 September 2017')
    bot.say('1 day || Reward drops below 1 LOG || 2 November 2018')
    bot.say('gamma day || Reward drops below gamma (~0.577) LOG || 21 August 2021 ')
    bot.say('dime day || Reward drops below 0.1 LOG || 28 March 2053')
    bot.say('penny day || Reward drops below 0.01 LOG  || 28 March 2396')
    bot.say('millie day || Reward drops below 0.001 LOG || 28 March 5816')
    
@commands('aboutholiday', 'holidays')
def aboutHolidays(bot, trigger):
    bot.say('We celebrate the forests no longer needing to be cleared on those merits alone. We hold no reverance for the cut-throat, unconscionable crew who regularly parody themselves in parlance. Holy days recorded by phm@woodcoin.org [BF3B 706A B29F 702F A8E1 060C 9776 11EC 0AA9 B111] @ 1459215887.')
    bot.say('For a complete list, say .halvening or .holiday')
...
def paper3(bot, trigger):
    ...
    bot.say('At any block, the reward continues to drop and a woodcutter will have an advantage over any future woodcutter. The incentive to chop wood in the present moment therefore remains and is not artificially diminished. ' + more2)
    
    
@commands('stillevenmore')
@priority('low')
@rule('$nickname stillevenmore2')
